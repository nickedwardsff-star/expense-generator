...
if len(st.session_state.expenses) > 0:
    ...
    # --- SORTING MAGIC ---
    # Convert shopping cart to a DataFrame and sort chronologically
    df = pd.DataFrame(st.session_state.expenses)
    df = df.sort_values(by="Date")
    # Change the date format back to UK standard (DD/MM/YYYY)
    ...
    try:
        ...
        for index, expense in enumerate(df.to_dict('records')):
            ...
            # 1. Write the standard text columns
            ws.cell(row=current_row, column=1, value=expense["Date"])         # Col A: Date
            ws.cell(row=current_row, column=2, value=expense["Vendor"])       # Col B: Expenditure
            ws.cell(row=current_row, column=3, value=expense["File Name"])    # Col C: Reason
            
            # 2. Split and write the Amount Excl VAT
            excl_pounds, excl_pence = split_pounds_pence(expense["Amount Excl VAT"])
            ws.cell(row=current_row, column=5, value=excl_pounds) # Col E
            ws.cell(row=current_row, column=6, value=excl_pence)  # Col F
            
            # 3. Split and write the VAT
            vat_pounds, vat_pence = split_pounds_pence(expense["VAT"])
            ws.cell(row=current_row, column=7, value=vat_pounds)  # Col G
            ws.cell(row=current_row, column=8, value=vat_pence)   # Col H
            
            # 4. Split and write the Total Amount
            total_pounds, total_pence = split_pounds_pence(expense["Total Amount"])
            ws.cell(row=current_row, column=9, value=total_pounds) # Col I
            ws.cell(row=current_row, column=10, value=total_pence) # Col J
